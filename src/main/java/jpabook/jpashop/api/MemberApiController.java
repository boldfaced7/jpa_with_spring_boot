...
public class MemberApiController {
    ...
    /**
     * 등록 V1: 요청값으로 Member 엔티티를 직접 받음
     * 문제점
     * - 엔티티에 프레젠테이션 계층을 위한 로직이 추가됨
     * - 엔티티에 API 검증을 위한 로직이 들어감(@NotEmpty 등등)
     * - 실무에서는 회원 엔티티를 위한 다양한 API가 만들어지는데, 한 엔티티에 각각의 API를 위한
     * 모든 요청 요구사항을 담기는 어려움
     * - 엔티티가 변경되면, API 스펙이 변함
     * <p>
     * 결론: API 요청 스펙에 맞추어 별도의 DTO를 파라미터로 받음
     */
     ...
     * - 응답 스펙을 맞추기 위한 로직이 추가됨(@JsonIgnore, 별도의 뷰 로직 등)
     * - 실무에서는 같은 엔티티에 대해 API가 용도에 따라 다양하게 만들어지는데,
     * 한 엔티티에 각각의 API를 위한 프레젠테이션 응답 로직을 담기는 어려움
     * <p>
     * - 엔티티가 변경되면, API 스펙이 변함
     * - 컬렉션을 직접 반환하면, 향후 API 스펙을 변경하기 어려움(별도의 Result 클래스 생성으로 해결)
     * <p>
     * - 결론: API 응답 스펙에 맞추어 별도의 DTO를 반환
     * <p>
     * 조회 V1은 안 좋은 버전으로, 모든 엔티티가 노출됨
     * - @JsonIgnore는 정말 최악(API가 이거 하나인가! 화면에 종속적이지 마라!)
     */
